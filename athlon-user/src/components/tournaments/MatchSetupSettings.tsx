...
import { MapPin, Phone, Calendar, Play, Save, Users, User, Trophy, Clock, CheckCircle2, AlertCircle, Sparkles } from "lucide-react";
...
export function MatchSetupSettings({ tournamentId }: MatchSetupSettingsProps) {
  ...
  // Local state for editing courts/umpires before saving
  const [editState, setEditState] = useState<{ [matchUuid: string]: { courtId?: string, umpirePhone?: string } }>({});
  ...
  const handleEditChange = (matchUuid: string, field: 'courtId' | 'umpirePhone', value: string) => {
    setEditState(prev => ({
      ...prev,
      [matchUuid]: {
        ...prev[matchUuid],
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 bg-surface border border-border/80 rounded-2xl shadow-sm">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            Assign courts and umpires to scheduled matches before launching live scoring.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {scheduledMatches.length === 0 ? (
          ...
          scheduledMatches.map((match, idx) => {
            ...
            const isEditing = edits.courtId !== undefined || edits.umpirePhone !== undefined;
