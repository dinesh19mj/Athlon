"use client";

import { useState, useEffect } from "react";
import { useParams, useRouter } from "next/navigation";
import { ArrowLeft, UserCheck, Shield, Check, DollarSign } from "lucide-react";
import Link from "next/link";
import { TeamChampionshipService, TeamChampionship } from "@/lib/api/teamChampionship";
import { useAuthStore } from "@/lib/store/useAuthStore";

export default function RegisterPlayerForChampionshipPage() {
  const params = useParams();
  const championshipUuid = params.id as string;
  const router = useRouter();
  const { userId, userUuid, userEmail } = useAuthStore();

  const [championship, setChampionship] = useState<TeamChampionship | null>(null);
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);

  const [formData, setFormData] = useState({
    fullName: "",
    phone: "",
    email: userEmail || "",
    categoryId: 0,
    categoryName: "",
    eligibleFormats: [] as string[],
  });

  useEffect(() => {
    TeamChampionshipService.getById(championshipUuid)
      .then((res) => {
        setChampionship(res);
        if (res?.categories && res.categories.length > 0) {
          setFormData((prev) => ({
            ...prev,
            categoryId: res.categories![0].categoryId || 0,
            categoryName: res.categories![0].name,
          }));
        }
      })
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [championshipUuid, userEmail]);

  const toggleFormat = (fmtName: string) => {
    setFormData((prev) => {
      const exists = prev.eligibleFormats.includes(fmtName);
      if (exists) {
        return { ...prev, eligibleFormats: prev.eligibleFormats.filter((f) => f !== fmtName) };
      } else {
        return { ...prev, eligibleFormats: [...prev.eligibleFormats, fmtName] };
      }
    });
  };

  const selectedCategory = championship?.categories?.find(
    (c) => c.categoryId === formData.categoryId || c.name === formData.categoryName
  );

  const registrationFee =
    championship?.playerFeeMode === "FREE"
      ? 0
      : championship?.playerFeeMode === "CATEGORY_PAID"
      ? selectedCategory?.registrationFee || championship?.defaultPlayerFee || 0
      : championship?.defaultPlayerFee || 0;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!championship?.championshipId) return;

    try {
      setSubmitting(true);
      await TeamChampionshipService.registerPlayer({
        championshipId: championship.championshipId,
        championshipUuid: championship.championshipUuid,
        userId: userId ? Number(userId) : undefined,
        userUuid: userUuid || undefined,
        fullName: formData.fullName,
        phone: formData.phone,
        email: formData.email,
        categoryId: formData.categoryId,
        categoryName: formData.categoryName,
        eligibleFormats: formData.eligibleFormats,
        basePrice: selectedCategory?.basePrice || 1000,
        feeAmount: registrationFee,
      });

      alert("Successfully registered for championship auction pool!");
      router.push(`/tournaments`);
    } catch (err: any) {
      alert(err.message || "Failed to register");
    } finally {
      setSubmitting(false);
    }
  };

  if (loading || !championship) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-foreground">
        <div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-background text-foreground pb-20 selection:bg-primary selection:text-black">
      <header
        className="sticky top-0 z-40 backdrop-blur-xl border-b px-4 sm:px-8 py-4 flex items-center gap-3"
        style={{ backgroundColor: "var(--athlon-surface)", borderColor: "var(--athlon-border)" }}
      >
        <Link
          href={`/tournaments`}
          className="p-2 rounded-xl border border-foreground/10 hover:bg-foreground/5 transition-all text-foreground/70"
        >
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <div>
          <h1 className="text-sm sm:text-base font-black text-foreground">Player Registration</h1>
          <p className="text-[11px] font-medium text-foreground/50">{championship.name}</p>
        </div>
      </header>

      <main className="max-w-lg mx-auto px-4 mt-6">
        <form
          onSubmit={handleSubmit}
          className="rounded-3xl border p-6 space-y-5 shadow-xl"
          style={{ backgroundColor: "var(--athlon-card)", borderColor: "var(--athlon-border)" }}
        >
          <div>
            <label className="block text-xs font-black uppercase tracking-wider text-foreground/70 mb-1.5">
              Full Name *
            </label>
            <input
              type="text"
              required
              value={formData.fullName}
              onChange={(e) => setFormData({ ...formData, fullName: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border bg-background text-sm font-semibold outline-none focus:border-primary"
              style={{ borderColor: "var(--athlon-border)" }}
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black uppercase tracking-wider text-foreground/70 mb-1.5">
                Phone *
              </label>
              <input
                type="tel"
                required
                value={formData.phone}
                onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                className="w-full px-4 py-3 rounded-xl border bg-background text-sm font-semibold outline-none focus:border-primary"
                style={{ borderColor: "var(--athlon-border)" }}
              />
            </div>
            <div>
              <label className="block text-xs font-black uppercase tracking-wider text-foreground/70 mb-1.5">
                Email
              </label>
              <input
                type="email"
                value={formData.email}
                onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                className="w-full px-4 py-3 rounded-xl border bg-background text-sm font-semibold outline-none focus:border-primary"
                style={{ borderColor: "var(--athlon-border)" }}
              />
            </div>
          </div>

          {/* Category Selector */}
          <div>
            <label className="block text-xs font-black uppercase tracking-wider text-foreground/70 mb-1.5">
              Classification Category *
            </label>
            <div className="grid grid-cols-2 gap-2">
              {championship.categories?.map((cat) => (
                <button
                  key={cat.categoryId}
                  type="button"
                  onClick={() => setFormData({ ...formData, categoryId: cat.categoryId!, categoryName: cat.name })}
                  className={`p-3 rounded-xl border text-left transition-all ${
                    formData.categoryName === cat.name
                      ? "bg-primary/10 border-primary text-primary font-black shadow-sm"
                      : "bg-surface border-foreground/10 text-foreground/70"
                  }`}
                >
                  <span className="text-xs block">{cat.name}</span>
                  <span className="text-[10px] text-foreground/40 font-semibold">
                    Base: {cat.basePrice || 1000} pts
                  </span>
                </button>
              ))}
            </div>
          </div>

          {/* Eligible Match Formats */}
          <div>
            <label className="block text-xs font-black uppercase tracking-wider text-foreground/70 mb-1.5">
              Eligible Match Formats
            </label>
            <div className="flex flex-wrap gap-2">
              {championship.matchFormats?.map((fmt) => {
                const selected = formData.eligibleFormats.includes(fmt.name);
                return (
                  <button
                    key={fmt.formatId}
                    type="button"
                    onClick={() => toggleFormat(fmt.name)}
                    className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-all ${
                      selected
                        ? "bg-primary text-primary-foreground border-primary"
                        : "bg-surface border-foreground/10 text-foreground/60 hover:border-foreground/30"
                    }`}
                  >
                    {selected ? "✓ " : "+ "}
                    {fmt.name}
                  </button>
                );
              })}
            </div>
          </div>

          {/* Fee & Submit */}
          <div className="border-t pt-4 space-y-3" style={{ borderColor: "var(--athlon-border-subtle)" }}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-foreground/50 uppercase">Registration Fee</span>
              <span className="text-sm font-black text-primary">
                {registrationFee > 0 ? `₹${registrationFee}` : "FREE"}
              </span>
            </div>

            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3.5 bg-primary text-primary-foreground font-black text-sm rounded-xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-primary/30 disabled:opacity-40"
            >
              {submitting ? "Registering..." : "Submit Player Registration"}
            </button>
          </div>
        </form>
      </main>
    </div>
  );
}
