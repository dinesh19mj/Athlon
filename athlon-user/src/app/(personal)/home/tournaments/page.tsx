      ...
      <main className="w-full max-w-lg mx-auto px-4 flex flex-col gap-6 pt-4">
        ...
        <div className="flex flex-col gap-5">
          {loading ? (
            ...
            return (
              <Link href={`/home/tournaments/${tournament.tournamentUuid}`} key={tournament.tournamentId} className="bg-surface border border-foreground/10 hover:border-foreground/30 rounded-[24px] overflow-hidden transition-colors shadow-lg cursor-pointer group block">
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {tournament.status === 'COMPLETED' || tournament.status === 'FINISHED' ? (
                        <span className="px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                          <Trophy className="w-2.5 h-2.5" /> Finished
                        </span>
                      ) : tournament.status === 'REGISTRATION_CLOSED' ? (
                        <span className="px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider bg-red-500/20 text-red-400 border border-red-500/30">
                          Closed
                        </span>
                      ) : null}
                  ...
                  <div className="flex items-center justify-between border-t border-foreground/5 pt-4">
                    ...
                    <div className="flex items-center justify-center gap-1 text-[10px] font-bold text-primary group-hover:text-foreground transition-colors bg-primary/10 px-3 py-1.5 rounded-lg">
                      {tournament.status === 'COMPLETED' || tournament.status === 'FINISHED' ? 'RESULTS' : 'DETAILS'} <ChevronRight className="w-3 h-3" />
                    </div>
                  </div>
