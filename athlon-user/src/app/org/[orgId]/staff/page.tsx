...
const STAFF_ROLES = [
  { value: 'MANAGER', label: 'Campus / Facility Manager', badgeClass: 'bg-amber-500/15 text-amber-300 border-amber-500/30' },
  { value: 'RECEPTIONIST', label: 'Front Desk & Reception', badgeClass: 'bg-primary/15 text-primary border-primary/30' },
  { value: 'OPERATIONS', label: 'Court & Operations Supervisor', badgeClass: 'bg-cyan-500/15 text-cyan-400 border-cyan-500/30' },
  { value: 'PHYSIOTHERAPIST', label: 'Physiotherapist & Medical', badgeClass: 'bg-rose-500/15 text-rose-400 border-rose-500/30' },
  { value: 'ACCOUNTANT', label: 'Accountant & Administration', badgeClass: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' },
  { value: 'INVENTORY_MANAGER', label: 'Equipment & Inventory', badgeClass: 'bg-violet-500/15 text-violet-400 border-violet-500/30' },
  { value: 'SUPPORT_STAFF', label: 'Support & Maintenance', badgeClass: 'bg-white/10 text-foreground/80 border-white/20' },
  { value: 'STAFF', label: 'General Staff', badgeClass: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
];
...
export default function StaffPage() {
  ...
  // Add Staff Modal State (Top-of-page Dialog)
  const [showAddModal, setShowAddModal] = useState(false);
  const [addPhone, setAddPhone] = useState('');
  const [addRole, setAddRole] = useState('MANAGER');
  const [addCentreUuid, setAddCentreUuid] = useState<string>('');
  ...
  // Load Staff Members & Campuses
  const loadData = async () => {
    if (!orgUuid) return;
    try {
      setLoading(true);
      const [staffRes, centresRes] = await Promise.allSettled([
        AcademyStaffService.getOperationalStaff(orgUuid),
        AcademyService.getCentres(orgUuid),
      ]);

      let staffData: AcademyStaffResponse[] = [];
      if (staffRes.status === 'fulfilled') {
        const raw = staffRes.value;
        staffData = Array.isArray(raw) ? raw : (raw as any)?.data || [];
      }

      setStaffList(staffData);

      if (centresRes.status === 'fulfilled') {
        const cList = centresRes.value || [];
        setCentres(cList);
        if (cList.length > 0 && !addCentreUuid) {
          setAddCentreUuid(cList[0].centreUuid);
        }
      }
    } catch (err) {
  ...
  const filteredStaff = useMemo(() => {
    return staffList.filter((s) => {
      ...
      const matchQuery =
        !q ||
        [s.fullName, s.phone, s.email, s.role, s.centreName].some((v) => v?.toLowerCase().includes(q));

      const matchRole =
        selectedRole === 'ALL' ||
        (s.role && s.role.toUpperCase() === selectedRole.toUpperCase());
  ...
  const handleVerifyPhone = async (phoneVal: string) => {
    ...
    try {
      ...
      if (user && (user.uuid || user.userId || user.userUuid)) {
        ...
        setVerifiedUser(null);
        setLookupError('No Athlon user found with this phone number. The staff member must have a registered Athlon account.');
      }
    } catch (err: any) {
      setVerifiedUser(null);
      setLookupError('No Athlon user found with this phone number. Please ask the staff member to create an Athlon account.');
    } finally {
  ...
  const openAddStaffModal = () => {
    setAddPhone('');
    setAddRole('MANAGER');
    setAddCentreUuid(centres[0]?.centreUuid || '');
    setVerifiedUser(null);
    setLookupError(null);
    setShowAddModal(true);
  };

  // Handle Add Staff Member to academy_staff table
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await AcademyStaffService.addStaff(orgUuid, {
        phone: addPhone.trim(),
        role: addRole,
        centreUuid: addCentreUuid || undefined,
      });

      const added = (res as any)?.data || res;
      showToast(`🎉 Staff member "${added.fullName || addPhone}" added successfully!`);
  ...
  const handleRemoveStaff = async (staff: AcademyStaffResponse) => {
    if (!confirm(`Are you sure you want to remove ${staff.fullName} from the staff roster?`)) return;
    try {
      const idToRemove = staff.staffUuid || (staff as any).organizationMemberUuid;
      if (idToRemove) {
        await AcademyStaffService.removeStaff(orgUuid, idToRemove);
      }
      setStaffList((prev) => prev.filter((m) => m.staffUuid !== idToRemove && (m as any).organizationMemberUuid !== idToRemove));
      showToast(`${staff.fullName} removed from staff.`);
    } catch (err: any) {
      console.error('Failed to remove staff member:', err);
      const msg = err?.response?.data?.message || err?.message || 'Could not remove staff member.';
      showToast(msg, true);
    }
  };
    ...
    <div className="min-h-screen bg-background pb-28">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-foreground/45 truncate">
                Operations &amp; Campus Administration • {orgName}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-1.5">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search staff by name, phone, department, campus..."
          ...
          <div className="relative shrink-0">
            ...
            >
              <option value="ALL" className="bg-[#18181b] text-white font-semibold">All Departments</option>
              {STAFF_ROLES.map((r) => (
      ...
      <div className="px-3.5 sm:px-4 pt-2 space-y-2.5">
        ...
        {!loading && (
          ...
          >
            <div>
              <div className="text-xs font-mono font-black text-primary">{staffList.length}</div>
              <div className="text-[8px] uppercase tracking-wider font-bold text-foreground/40">Total Staff</div>
            </div>
            <div>
              <div className="text-xs font-mono font-black text-amber-400">
                {staffList.filter((s) => (s.role || '').toUpperCase().includes('MANAGER') || (s.role || '').toUpperCase().includes('ADMIN')).length}
              </div>
              <div className="text-[8px] uppercase tracking-wider font-bold text-foreground/40">Management</div>
            </div>
            <div>
              <div className="text-xs font-mono font-black text-cyan-400">
                {centres.length}
              </div>
              <div className="text-[8px] uppercase tracking-wider font-bold text-foreground/40">Campuses</div>
            </div>
          </div>
        )}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center min-h-[220px] gap-2.5">
            ...
            <p className="text-xs text-foreground/40 font-medium">Fetching staff roster...</p>
          </div>
            ...
            <div>
              ...
              <p className="text-[11px] text-foreground/45 mt-0.5 max-w-xs">
                {searchTerm
                  ...
                  : 'Tap the "+" button below to register campus managers, receptionists, supervisors, and administrative staff.'}
              </p>
            </div>
            {canManage && (
              <button
          ...
          <div className="space-y-2.5">
            {filteredStaff.map((staff) => {
              const isExpanded = expandedId === staff.staffUuid;
              const cleanPhone = (staff.phone || '').replace(/[^0-9]/g, '');
              const roleObj =
                STAFF_ROLES.find((r) => r.value === (staff.role || '').toUpperCase()) || {
                  value: staff.role || 'STAFF',
                  label: staff.role || 'Staff Member',
                  badgeClass: 'bg-primary/10 text-primary border-primary/20',
                };

              return (
                <div
                  key={staff.staffUuid || staff.phone}
                  ...
                  <div className="p-3">
                    <div className="flex items-start justify-between gap-2.5">
                      <div className="flex items-start gap-2.5 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {/* Contact and Department Subtitle */}
                          <div className="flex items-center gap-2 mt-0.5 text-[10px] text-foreground/50 flex-wrap">
                            ...
                            {staff.phone && (
                              <>
                                <span>•</span>
                                <span className="font-mono text-foreground/60">{staff.phone}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      {/* Expand Toggle */}
                      <button
                        onClick={() => setExpandedId(isExpanded ? null : staff.staffUuid)}
                    ...
                    <div className="flex items-center justify-between gap-2 mt-2.5 pt-2 border-t border-white/5">
                      <div className="flex items-center gap-1">
                        ...
                        {cleanPhone && (
                          <a
                            href={`https://wa.me/${cleanPhone.startsWith('91') ? cleanPhone : '91' + cleanPhone}?text=${encodeURIComponent(
                              `Hello ${staff.fullName}, regarding academy operations at ${orgName}.`
                            )}`}
                            target="_blank"
                        ...
                        {staff.email && (
                          <a
                            href={`mailto:${staff.email}?subject=${encodeURIComponent(
                              `Academy Operations - ${orgName}`
                            )}`}
                    ...
                    <div className="border-t space-y-2 p-3 bg-black/20" style={{ borderColor: 'var(--athlon-border)' }}>
                      <div className="space-y-1 text-[10px]">
                        <div className="flex items-center justify-between">
                          <span className="text-foreground/40">Email Address</span>
                          <span className="text-foreground truncate max-w-[200px]">{staff.email || 'Not provided'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-foreground/40">Staff ID</span>
                          <span className="font-mono text-foreground/70">{staff.staffId || '#'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-foreground/40">Campus / Centre</span>
                          <span className="text-foreground">{staff.centreName || 'All Campuses'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-foreground/40">Status</span>
                          <span className="text-emerald-400 font-bold">{staff.status || 'Active'}</span>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          TOP-PLACED DIALOG MODAL: ADD STAFF VIA PHONE VERIFICATION
         ══════════════════════════════════════════════════════════════════ */}
      {showAddModal && (
        <div
            ...
            <form onSubmit={handleAddStaff} className="p-4 space-y-3.5">
              ...
              <div>
                <label className="block text-[10px] font-bold text-foreground/50 uppercase tracking-wider mb-1">
                  Staff Phone Number <span className="text-primary">*</span>
                </label>
                ...
                <p className="text-[10px] text-foreground/40 mt-1">
                  Verifies registered Athlon account for operations/facility staff.
                </p>
              </div>
              ...
              {verifiedUser && (
                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/25 flex items-center gap-3 animate-in fade-in">
                  <div className="w-10 h-10 rounded-xl bg-emerald-500/20 text-emerald-300 font-black text-xs flex items-center justify-center shrink-0 border border-emerald-500/30">
                    {(verifiedUser.firstName || verifiedUser.name || 'S').slice(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {/* Assigned Campus Dropdown if available */}
              {centres.length > 0 && (
                <div>
                  <label className="block text-[10px] font-bold text-foreground/50 uppercase tracking-wider mb-1">
                    Assigned Campus / Venue
                  </label>
                  <div className="relative">
                    <select
                      value={addCentreUuid}
                      onChange={(e) => setAddCentreUuid(e.target.value)}
                      className="w-full px-3 py-2 bg-background/60 border border-white/10 rounded-xl text-xs text-foreground focus:outline-none focus:border-primary/60 transition appearance-none cursor-pointer"
                    >
                      {centres.map((c) => (
                        <option key={c.centreUuid} value={c.centreUuid} className="bg-[#18181b] text-white font-medium">
