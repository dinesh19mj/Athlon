#!/bin/bash
echo -e '[Unit]\nDescription=Auth Service\nAfter=network.target\n\n[Service]\nUser=ubuntu\nExecStart=/usr/bin/java -jar /home/ubuntu/services/AUTHSERVICE-0.0.1-SNAPSHOT.jar\nSuccessExitStatus=143\nRestart=always\nRestartSec=5\n\n[Install]\nWantedBy=multi-user.target' | sudo tee /etc/systemd/system/authservice.service
